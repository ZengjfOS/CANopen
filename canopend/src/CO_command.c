...
static void* command_thread(void* arg) {
    ...
    /* Almost endless loop */
    // 从这里基本上可以看出，目前系统只支持一个Socket连接，不能在系统上同时打开两个CAN设备
    while(endProgram == 0) {
...
static void command_process(int fd, char* command, size_t commandLength) {
    ...
    // 获取nodeId，获取之后i为1，后面的switch中i=1，同时token将指向下一字符串(read/write)
    if(err == 0) {
        for(i=0; i<3; i++) {
            if((token = getTok(NULL, spaceDelim, &err)) == NULL) {
                break;
            }
            // isdigit，如果不是数字，返回0，是数字，返回非零
    ...
    if(err == 0) {
        ...
        case 1: /* <node> and <command> tokens */
            if(ui[0] < 0 || ui[0] > 127) {                      // 检查nodeId
                err = 1;
                respErrorCode = respErrorUnsupportedNode;
            }
            else {
                comm_node = (uint8_t) ui[0];                    // 通信nodeId
            }
            break;
