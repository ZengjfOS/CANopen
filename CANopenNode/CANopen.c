...
// 如果被定义，则采用全局变量的形式，否则将采用calloc的形式，这里需要注意的就是
// 运行的系统是否支持calloc()函数的问题。
...
/* Indexes for CANopenNode message objects ************************************/
    ...
    #define CO_RXCAN_NMT       0                                      /* 0          index for NMT message */
    #define CO_RXCAN_SYNC      1                                      /* 1          index for SYNC message */
    #define CO_RXCAN_RPDO     (CO_RXCAN_SYNC+CO_NO_SYNC)              /* 1 + 2  = 3 start index for RPDO messages */
    #define CO_RXCAN_SDO_SRV  (CO_RXCAN_RPDO+CO_NO_RPDO)              /* 3 + 16 = 19 start index for SDO server message (request) */
    #define CO_RXCAN_SDO_CLI  (CO_RXCAN_SDO_SRV+CO_NO_SDO_SERVER)     /* 19 + 1 = 20 start index for SDO client message (response) */
    #define CO_RXCAN_CONS_HB  (CO_RXCAN_SDO_CLI+CO_NO_SDO_CLIENT)     /* 20 + 1 = 21 start index for Heartbeat Consumer messages */
    /* total number of received CAN messages */
    #define CO_RXCAN_NO_MSGS (1+CO_NO_SYNC+CO_NO_RPDO+CO_NO_SDO_SERVER+CO_NO_SDO_CLIENT+CO_NO_HB_CONS) /* (1 + 1 + 16 + 1 + 1 + 4) = 24 */

    #define CO_TXCAN_NMT       0                                      /* 0           index for NMT master message */
    #define CO_TXCAN_SYNC      CO_TXCAN_NMT+CO_NO_NMT_MASTER          /* 0 + 1 = 1   index for SYNC message */
    #define CO_TXCAN_EMERG    (CO_TXCAN_SYNC+CO_NO_SYNC)              /* 1 + 1 = 2   index for Emergency message */
    #define CO_TXCAN_TPDO     (CO_TXCAN_EMERG+CO_NO_EMERGENCY)        /* 2 + 1 = 3   start index for TPDO messages */
    #define CO_TXCAN_SDO_SRV  (CO_TXCAN_TPDO+CO_NO_TPDO)              /* 3 + 16 = 19 start index for SDO server message (response) */
    #define CO_TXCAN_SDO_CLI  (CO_TXCAN_SDO_SRV+CO_NO_SDO_SERVER)     /* 19 + 1 = 20 start index for SDO client message (request) */
    #define CO_TXCAN_HB       (CO_TXCAN_SDO_CLI+CO_NO_SDO_CLIENT)     /* 20 + 1 = 21 index for Heartbeat message */
    /* total number of transmitted CAN messages */
    #define CO_TXCAN_NO_MSGS (CO_NO_NMT_MASTER+CO_NO_SYNC+CO_NO_EMERGENCY+CO_NO_TPDO+CO_NO_SDO_SERVER+CO_NO_SDO_CLIENT+1) /* (1 + 1 + 1 + 16 + 1 + 1 + 1) = 22  */
        ...
        uint16_t                bitRate)
{
    ...
    // 校验CO_OD中的TPDO、RPDO参数宏定义是否符合要求，主要是检查定义的结构体长度
    ...
    #if CO_NO_SDO_CLIENT == 1
    // 校验CO_OD中的SDOClient参数宏定义是否符合要求，主要是检查定义的结构体长度
    ...
    // }CO_t;
    //
    // CO_t类型对象会包含所有的CANopen协议栈的对象，控制了所有的结构体对象。
    CO = &COO;

    CO->CANmodule[0]                    = &COO_CANmodule;                   // CAN模块
    CO_CANmodule_rxArray0               = &COO_CANmodule_rxArray0[0];       // CAN模块的接收数组
    CO_CANmodule_txArray0               = &COO_CANmodule_txArray0[0];       // CAN模块的发送数组
    // #define CO_NO_SDO_SERVER               1   //Associated objects: 1200
  ...
  //#define CO_NO_SDO_CLIENT               1   //Associated objects: 1280
    ...
    /**
     * 接下来主要是根据当前参数，初始化CO对象中的定义的各个模块，同时会初始化各个模块对应的接收、发送信息的rxArray/txArray。
     */
    ...
    // CANopen SDO object模块初始化
    for (i=0; i<CO_NO_SDO_SERVER; i++)
    {
        // 从这里可以看出每个SDO本身占用了两个COB_ID
        uint32_t COB_IDClientToServer;
        uint32_t COB_IDServerToClient;
        if(i==0){
            /*Default SDO server must be located at first index*/
            // COB_ID = nodeId + offset(这个offset可以认为是网络编程的端口号)
            // CO_CAN_ID_RSDO = 0x600,   /**< 0x600 = 0b11000000000, SDO request from client (+nodeID) */
            COB_IDClientToServer = CO_CAN_ID_RSDO + nodeId;
            // CO_CAN_ID_TSDO = 0x580,   /**< 0x580 = 0b10110000000, SDO response from server (+nodeID) */
        ...
        err = CO_SDO_init(
                CO->SDO[i],                     // SDO对象
                COB_IDClientToServer,           // client
                COB_IDServerToClient,           // server
                OD_H1200_SDO_SERVER_PARAM+i,    // 目前还看不懂这个，应该是CANopen标准中的内容
                i==0 ? 0 : CO->SDO[0],
               &CO_OD[0],                       // 对象字典第一个入口
                CO_OD_NoOfElements,             // #define CO_OD_NoOfElements 169 也就是对象字典中有多少个入口了
                CO_SDO_ODExtensions,
                nodeId,
                CO->CANmodule[0],               // 传入所属的CAN模块
                CO_RXCAN_SDO_SRV+i,             // start index for SDO server message
                CO->CANmodule[0],               // 传入所属的CAN模块
                CO_TXCAN_SDO_SRV+i);            // start index for SDO server message
    }

    if(err){CO_delete(CANbaseAddress); return err;}


    // CANopen Emergency report object、Emergency process object模块初始化
    err = CO_EM_init(
            CO->em,
            CO->emPr,
            CO->SDO[0],
    ...
    // CANopen Heartbeat consumer object模块初始化
...
#if CO_NO_SDO_CLIENT == 1
    // CANopen SDO client object模块初始化
...
#if CO_NO_TRACE > 0
    // CANopen Trace object for monitoring variables模块初始化
    for(i=0; i<CO_NO_TRACE; i++) {
        CO_trace_init(
            CO->trace[i],
            CO->SDO[0],
