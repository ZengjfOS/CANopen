        ...
        uint16_t                CANbitRate)
{
    ...
    if(ret == CO_ERROR_NO && CANmodule->wasConfigured == 0){
        ...
        // 在Linux系统上，使用socketCAN来做驱动，这样，发送、接收数据基本上和普通的网络编程是一个概念了。
...
void CO_CANrxWait(CO_CANmodule_t *CANmodule){
    ...
    if(CANmodule->CANnormal){
        if(n != size){
            ...
            // rxArray中是当前模块对应的nodeId和其对应的offset(port)
