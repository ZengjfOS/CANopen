...
#ifndef CO_SINGLE_THREAD
pthread_mutex_t             CO_CAN_VALID_mtx = PTHREAD_MUTEX_INITIALIZER; // PTHREAD_MUTEX_INITIALIZER 不是一个变量，是一个初始化宏
#endif
...
static void sigHandler(int sig) {
    CO_endProgram = 1;                          /* 进程结束标志，用于同步结束所有的相关线程 */
}
...
int main (int argc, char *argv[]) {
    CO_NMT_reset_cmd_t reset = CO_RESET_NOT;                    /* 初始化为normal reutrn模式 */
    CO_ReturnError_t odStorStatus_rom, odStorStatus_eeprom;
    int CANdevice0Index = 0;
    int opt;
    bool_t firstRun = true;                                     /* 首次运行 */
    ...
    while((opt = getopt(argc, argv, "i:p:rc:s:a:")) != -1) {
        switch (opt) {
            case 'i':   // 解析-i参数，获取nodeId
                nodeId = strtol(optarg, NULL, 0);
                nodeIdFromArgs = true;
                break;
            case 'p': rtPriority = strtol(optarg, NULL, 0); break;  // 获取设定的优先级
            case 'r': rebootEnable = true;                  break;  // 打开重启功能
#ifndef CO_SINGLE_THREAD
            case 'c':   // 打开本地socket功能
                ...
                commandEnable = true;                               // 使能命令行功能
                break;
#endif
            case 's': odStorFile_rom = optarg;              break;  // 对象字典ROM文件名
            case 'a': odStorFile_eeprom = optarg;           break;  // 对象字典EEPROM文件名
            default:
                printUsage(argv[0]);
                exit(EXIT_FAILURE);
        }
    ...
    // OD_powerOnCounter是自动增长的变量，被存储于非易失性存储空间中
    printf(", count=%u ...\n", ++OD_powerOnCounter);
...
/* CANopen communication reset - initialize CANopen objects *******************/
        ...
        if(!firstRun) {
            CO_LOCK_OD();
            // CO第一次不是NULL么？这里目前看不懂，CO不是还没初始化么？
            // 主要是因为firstRun这个默认值是True，所以while循环第一次不会运行这部分，
            // 在第二次的while循环才会运行到这里，而第一次while循环，CO就会被初始化了，
            // 也就没有问题了。
        ...
        if(!nodeIdFromArgs) {
            ...
            // 如果命令行参数中没有CAN节点的ID，那么就使用对象字典中的值，在单片机上这是很实用的技巧
            nodeId = OD_CANNodeID;
        }
        // 初始化CAN设备，主要是对CO对象进行初始化
